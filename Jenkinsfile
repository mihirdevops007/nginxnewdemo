    ...
    steps {
     ...
     //        sh "docker push ${ECR_REPOSITORY}:${ECR_IMAGE_VERSION}"		
            withAWS(region: "${AWS_DEFAULT_REGION}", credentials: registryCredential) {
                sh "aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin 514523777807.dkr.ecr.us-east-1.amazonaws.com/nginxdemo" // Authenticate with ECR
