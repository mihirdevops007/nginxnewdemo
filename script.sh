...
if [ $? -eq 0 ]; then
    ...
    if [ "$REVISION" != "null" ]; then
        ...
        aws ecs update-service --cluster "$CLUSTER_NAME" --service "$SERVICE_NAME" --task-definition "$TASK_DEFINITION_NAME:$REVISION" --desired-count "$DESIRED_COUNT" --execution-role-arn "arn:aws:iam::514523777807:role/JeenkinsCICD_EC2"
    else
